#!/usr/bin/env python3
"""Decode throughput benchmark for the Ornith recipe (mirrors bench-miaai.py).

Same methodology as the DS4 recipe's bench-miaai.py (unique cold prefix,
min_tokens=max_tokens=128, ignore_eos, thinking off) so Ornith and DS4
numbers are directly comparable. Supports the same auth: --api-key arg or
$ORNITH_API_KEY env (empty = no auth).

Usage:
  python3 bench-ornith.py [--base-url http://127.0.0.1:8889/v1]
      [--model ornith-1.5-35b-a3b-nvfp4] [--prompt 256] [--concurrency 1] [--repeat 5]
"""
...
async def main():
    ...
    ap.add_argument("--api-key", default="", help="Bearer API key (default: $ORNITH_API_KEY)")
    args = ap.parse_args()
